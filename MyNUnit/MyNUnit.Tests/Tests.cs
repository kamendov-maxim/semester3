...
public class Tests
{
    ...
    {
        IEnumerable<FailedTestResult> res =
                _results.Where(r => r.GetType() == typeof(FailedTestResult)).Select(r => (FailedTestResult)r);
        Assert.That(res.Count, Is.EqualTo(_failedTests));
        foreach (var r in res)
        {
            if (r.Name != "TestShouldFail" && r.Name != "IncorrectException")
            {
                Assert.Fail();
            }
    ...
    {
        IEnumerable<FinishedWithExceptionResult> res =
            _results.Where(r => r.GetType() == typeof(FinishedWithExceptionResult))
            .Select(r => (FinishedWithExceptionResult)r);
    ...
    {
        IEnumerable<IgnoredTestResult> res =
            _results.Where(r => r.GetType() == typeof(IgnoredTestResult))
            .Select(r => (IgnoredTestResult)r);
